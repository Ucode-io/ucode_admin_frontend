...
const StatusMenuList = (props) => {
  ...
  if (type !== "status" && type !== "STATUS") return <components.MenuList {...props} />;

  const groups = [
    { label: "To do", options: field?.attributes?.todo?.options || [] },
    { label: "In Progress", options: field?.attributes?.progress?.options || [] },
...
const AddOptionBlock = ({ field, dialogState, handleClose, addNewOption }) => {
  ...
  return (
    <div style={{ padding: "20px", width: "400px" }}>
      <h2 style={{ marginBottom: "15px", fontSize: "16px", fontWeight: "bold" }}>Add option</h2>
      <div style={{ display: "flex", gap: "10px", marginBottom: "15px" }}>
        <div >
          ...
          {hasColor && <h4 style={{ fontSize: "12px", marginTop: "5px" }}>Color</h4>}
        </div>
        <div>
          {hasIcon && (
            <HFIconPicker shape="rectangle" control={control} name="icon" />
          )}
          {hasIcon && <h4 style={{ fontSize: "12px", marginTop: "5px" }}>Icon</h4>}
        </div>
      </div>
      <form>
        <div style={{ marginBottom: "10px" }}>
          <FRow label="Label">
            <HFTextField defaultValue="" control={control} name="label" fullWidth />
          </FRow>
        </div>
        <div style={{ marginBottom: "15px" }}>
          <FRow label="Value">
            <HFTextField defaultValue="" control={control} name="value" fullWidth />
          </FRow>
        </div>
      </form>
...
const CSelectBase = ({
  ...
  const isMultiselectType = currentField?.type === "multiselect" || currentField?.type === "MULTISELECT" || currentField?.attributes?.is_multiselect;
  const isStatusType = currentField?.type === "status" || currentField?.type === "STATUS";

  // Use hook logic
  const { options, isFetching, handleSearch, setPage, refetch } = useSelectLogic({
    field: currentField,
    type: isMultiselectType ? "multiselect" : isStatusType ? "status" : "relation",
    value,
    autoFiltersValue,
    row
  });
  ...
  const selectValue = useMemo(() => {
    ...
    if (isMulti || isMultiselectType) {
      ...
      return valArray.map((v) => findOption(v) || { label: v, value: v }).filter(Boolean);
    }
  ...
  useEffect(() => {
    ...
    const selectedObj = typeof selectedId === 'object' ? selectedId : options.find(o => (o.guid === selectedId || o.value === selectedId || o.slug === selectedId));

    if (selectedObj) {
      currentField.attributes.autofill.forEach(({ field_from, field_to, automatic }) => {
        const path = name?.includes('.') ? `${name.split('.').slice(0, -1).join('.')}.${field_to}` : field_to;
        if (automatic) {
          setTimeout(() => {
  ...
  const customStyles = {
    ...
    option: (base, state) => {
      ...
      return {
        ...
        fontSize: "13px",
        "&:hover": {
          backgroundColor: (color && !isBadgeType) ? color : "rgba(242, 241, 238, 0.6)",
    ...
    <components.SingleValue {...props}>
      <div style={{ display: "flex", alignItems: "center", width: '100%' }}>
          ...
          >
            {currentField?.attributes?.has_icon && props.data.icon && (
              <span style={{ marginRight: '6px', display: 'flex' }}>
                {props.data.icon?.includes(':') ? <IconGeneratorIconjs icon={props.data.icon} size={14} /> : <IconGenerator icon={props.data.icon} size={14} />}
              </span>
            )}
            {props.children}
          </span>
        ...
        {!disabled && !isStatusType && !isMultiselectType && (
          <Box
            sx={{ marginLeft: '5px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
      ...
      return (
        <div
          style={{
            display: 'flex',
            alignItems: 'center',
            background: data.color ? `${data.color}30` : '#e0e0e0',
        ...
        >
          {currentField.attributes?.has_icon && (
            <span style={{ marginRight: '4px', display: 'flex' }}>
              {data.icon?.includes(':') ? <IconGeneratorIconjs icon={data.icon} size={14} /> : <IconGenerator icon={data.icon} size={14} />}
            </span>
          )}
  ...
  return (
    <>
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
        ...
        {currentField?.attributes?.creatable && !isMultiselectType && !isStatusType && !value && variant !== 'table' && (
          <span
        ...
        <div style={{ width: '100%' }}>
          <Select
            ...
            onMenuScrollToBottom={() => {
              ...
              // Only load more if not all loaded
              // We don't have total count in CSelectBase scope easily unless we request it from useSelectLogic?
              // useSelectLogic returns options, but maybe we should return count or hasMore?
              // For now, just increment page, useQuery will handle it? 
              // unique check in useSelectLogic prevents duplicates.
              setPage(p => p + 1)
            }}
            getOptionLabel={(opt) => {
              if (currentField?.attributes?.enable_multi_language) {
                return opt[`label_${i18n.language}`] || opt[`name_${i18n.language}`] || opt.label || opt.name || opt.value || "";
              }
              return opt[`label_${i18n.language}`] || opt.label || opt.name || opt.value || "";
            }}
            getOptionValue={(opt) => opt.guid || opt.value || opt.slug || opt.id}
            onChange={(newValue) => {
              if (isMulti || isMultiselectType) {
                ...
                const val = newValue?.guid || newValue?.slug || newValue?.value;
                onChange(val, newValue);
              }
              if (updateObject) updateObject();
            }}
            components={{
              MenuList: (props) => <StatusMenuList {...props} type={currentField?.type} field={currentField} />,
              Option: CustomOption,
              ...
              ClearIndicator: (props) => (
                <components.ClearIndicator {...props}>
                  <CloseIcon style={{ fontSize: '14px', cursor: 'pointer' }} />
              ...
              if (currentField?.attributes?.creatable) {
                if (isMultiselectType && inputValue) {
                  return (
                    ...
                    >
                      <AddIcon style={{ fontSize: 16, verticalAlign: 'middle' }} /> Create {inputValue}
                    </div>
                  )
                }
                if (!isMultiselectType && !isStatusType) {
                  ...
                  return (
                    <div
                      style={{ cursor: 'pointer', color: '#007AFF' }}
                      onClick={() => {
                        navigateToForm(currentField?.table_slug, "CREATE", {}, {}, null);
                      }}
                    >
                      <AddIcon style={{ fontSize: 16, verticalAlign: 'middle' }} /> Create {inputValue ? `"${inputValue}"` : "new"}
                    </div>
                  )
                }
              }
      ...
      {modalTableSlug && (
        <ModalDetailPage
          ...
          handleClose={() => { setModalOpen(false); setModalTableSlug(null); }}
        />
      )}
    </>
  );
...
const CSelectControlled = ({ control, name, rules, row, field, handleChange, ...props }) => {
  const currentField = field || row;
  ...
  const combinedAutoFilters = { ...props.autoFiltersValue, ...autoFiltersValue };
    ...
    <Controller
      ...
      render={({ field: { onChange, value }, fieldState: { error } }) => (
        <>
          ...
          />
          {error && <p style={{ color: 'red', fontSize: '11px', margin: '2px 0 0 0' }}>{error.message}</p>}
        </>
      )}
    />
  );
