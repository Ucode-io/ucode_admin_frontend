...
export default function App({ routes = [], isShowBack = true }) {
  ...
  return (
    <div className="flex">
      ...
      {routes.map((el, i) => (
        <div key={Math.random()} className="flex items-center text-sm">
          <div
            key={Math.random()}
          ...
          >
            ...
            {routes.length !== i + 1 ? (
              <span
                ...
                onClick={() => (el.link ? history.push(el.route) : {})}
              >
                {el.title}
              </span>
            ) : (
            ...
            {routes.length !== i + 1 ? (
              <div className="mr-3 ml-1">{arrow}</div>
            ) : (
              ""
            )}
          </div>
