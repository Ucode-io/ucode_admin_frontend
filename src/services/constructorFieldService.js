...
const constructorFieldService = {
  getList: (params, tableSlug) =>
    requestV2.get(`/fields/${params.tableSlug ?? tableSlug}`, {params}),
...
export const useFieldCreateMutation = (mutationSettings) => {
  return useMutation(
    ({data, tableSlug}) => constructorFieldService.create({data, tableSlug}),
    mutationSettings
  );
};
export const useFieldUpdateMutation = (mutationSettings) => {
  return useMutation(
    ({data, tableSlug}) => constructorFieldService.update({data, tableSlug}),
    mutationSettings
  );
};
