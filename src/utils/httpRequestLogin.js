...
const httpRequestLogin = axios.create({
  baseURL,
  timeout: 100000,
});
  ...
  if (error?.response?.status === 401) {
    ...
    return authService
      ...
      .then((res) => {
        ...
        return httpRequestLogin(originalRequest);
      })
      .catch((err) => {
        console.log(err);
...
httpRequestLogin.interceptors.request.use(
  (config) => {
...
httpRequestLogin.interceptors.response.use((response) => {
  customMessageHandler(response);
  return response.data.data;
}, errorHandler);

export default httpRequestLogin;
