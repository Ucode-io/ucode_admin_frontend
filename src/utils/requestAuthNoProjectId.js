...
const requestAuthNoProjectId = axios.create({
  baseURL,
  timeout: 100000,
});
...
requestAuthNoProjectId.interceptors.request.use(
  (config) => {
...
requestAuthNoProjectId.interceptors.response.use(
  (response) => response.data.data,
  errorHandler
);

export default requestAuthNoProjectId;
