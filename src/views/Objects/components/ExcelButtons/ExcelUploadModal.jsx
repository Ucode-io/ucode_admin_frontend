...
import { CloseIcon, PointerIcon, UploadIcon } from "../../../../assets/icons/icon";
...
const ExcelUploadModal = ({ fieldsMap, handleClose }) => {
  ...
  const onUpload = async (e) => {
    ...
    try {
      ...
      const excelResponse = await excelService.getExcel({excel_id: res?.filename.replace(/\.[^.$]+$/, '')})
  ...
  const onSubmit = (values) => {
    ...
    excelService.upload(fileName.replace(/\.[^.$]+$/, ''),{ 
        data: {
           ...computedData 
        },
          ...
          <Tabs className={styles.tabs_head} selectedIndex={tabIndex}>
            <TabList className={styles.tabs_list}>
              ...
              <Tab className={styles.tabs_item}><span>Подтверждения</span></Tab>
                <button onClick={handleClose} className={styles.tabs_close}><CloseIcon style={{ color:'#6E8BB7' }}/></button>
            </TabList>
              ...
              <TabPanel >
                <div className={styles.dialog_upload}>
                    {visible ? (
                        ...
                        <input type="file" id="file" ref={inputFIle} style={{ display: 'none' }} onChange={onUpload} />
                        ...
                        <div className={styles.upload_select}>
                         ...
                         {fields?.sort((first, second) => (second.required === true) - (first.required === true)).map((item, index) => (
                          ...
                          <div className={styles.control_btns}>
                             <button className={styles.control_clear} onClick={() => clearBtn()}>Сбросить</button>
                             <button className={styles.control_upload} onClick={handleSubmit(onSubmit)}>{btnLoader ?   
                               <span className={styles.btn_loader}><RippleLoader size="btn_size" height='20px'/></span> : 
                             'Загрузить'}</button>
                           </div>
                        </div>
                    </div>
