...
import { historyStatuses, currentStatuses } from "../../../../constants/statuses"
...
export default function OrderClient({ customer_id }) {
  ...
  const fetData = async (page) => {
    try {
      const current = await getOrders({ limit: 10, page, status_ids: currentStatuses.ids, customer_id })
      // setCounts({ cur: current.count })
      const history = await getOrders({ limit: 10, page, status_ids: historyStatuses.ids, customer_id })
        ...
        <TableContainer className="mt-4 rounded-lg border border-lightgray-1">
          <Table aria-label="simple table">
            ...
            <TableBody>
              {items.data &&
              items.data.length ?
                items.data.map(({ id, external_order_id, created_at, to_address, order_amount, steps, delivery_type, source }, index) => (
                  <TableRow className={index % 2 === 0 ? "bg-lightgray-5" : ""} key={id}>
                    <TableCell>
                    ...
                    <TableCell><Tag color="yellow" className="p-1">{delivery_type}</Tag></TableCell>
                    <TableCell><Tag color="purple" className="p-1">{source}</Tag></TableCell>
                  </TableRow>
                )) : <></>
              }
            </TableBody>
