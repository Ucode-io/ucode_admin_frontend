...
import { getOneBranchUser, postBranchUser, updateBranchUser } from "../../../../../../services/branchUsers"
...
export default function CreateUser() {
  ...
  const getItem = () => {
    ...
    getOneBranchUser(user_id).then(res => {
      formik.setValues({
        name: res.name,
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setUserRoles(user_roles ? user_roles.map(elm => ({ label: elm.name, value: elm.id })) : [])
  ...
  const saveChanges = (data) => {
    setSaveLoading(true)
    const selectedAction = user_id ? updateBranchUser(user_id, data) : postBranchUser(data)
    selectedAction
      .then(() => {
        history.goBack()
      })
                ...
                <div className="flex">
                  ...
                  <div className="w-3/4">
                    <Form.Item formik={formik} name="name">
                      <Input size="large" id="name" value={values.name} onChange={handleChange} />
                    </Form.Item>
                  </div>
                </div>
              </div>
                ...
                <div className="flex">
                  ...
                  <div className="w-3/4">
                    <Form.Item formik={formik} name="phone">
                      <Input size="large" prefix="+998" id="phone" type="number" value={values.phone} onChange={handleChange} />
                    </Form.Item>
                  </div>
                </div>
              </div>
