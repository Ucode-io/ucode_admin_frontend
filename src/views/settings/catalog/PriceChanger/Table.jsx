...
export default function MainTable({ createModal, setCreateModal, search }) {
  ...
  const getItems = (page) => {
    setLoader(true);
    getPriceChangers({ limit, page, search })
      .then((res) => {
        setItems({
          count: res.count,
          data: res.price_changers,
        });
      })
  ...
  const handleDeleteItem = () => {
    ...
    deletePriceChanger(deleteModal.id)
      .then((res) => {
        getItems(currentPage);
  ...
  const onSubmit = (values) => {
    ...
    const selectedAction = createModal.id
      ? updatePriceChanger(createModal.id, data)
  ...
  const initialColumns = [
    ...
    },
    {
      title: t("amount"),
      key: "amount",
      render: (record) => t(record.amount),
    },
    {
      title: t("base"),
      key: "base",
      render: (record) => t(record.base),
    },
    {
      title: t("change_type"),
      key: "change_type",
      render: (record) => t(record.change_type),
    },
  ];
              ...
              items?.data?.map((elm, index) => (
                <TableRow
                  ...
                  onClick={() => {
                    ...
                    history.push(`/home/catalog/price-changers/${elm.id}`);
                  }}
