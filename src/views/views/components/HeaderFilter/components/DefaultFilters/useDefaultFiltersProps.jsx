...
export const useDefaultFiltersProps = ({ handleClosePopover }) => {
  ...
  const allFields = Object.values(fieldsMap).filter((el) => {
    return el.type === FIELD_TYPES.LOOKUP
  ...
  const filterGenerator = (field) => {
    ...
    const statusOptions = [...todoOptions, ...completeOptions, ...progressOptions].map((option) => {
      return {
        ...option,
    ...
    switch(field.type) {
      case FIELD_TYPES.STATUS:
        return <FilterDropdown defaultValue={statusDefaultValue} onChange={(value) => handleChange(value, field.slug)} options={statusOptions} placeholder={"Filter by"} multiple  />
      case FIELD_TYPES.MULTISELECT:
        return <FilterDropdown defaultValue={multiSelectDefaultValue} onChange={(value) => handleChange(value, field.slug)} options={multiSelectOptions} placeholder={"Filter by"} multiple  />
      case FIELD_TYPES.LOOKUP:
        return <DefaultFilterRelation defaultValue={defaultFiltersMap[field.slug]} field={field} handleChange={handleChange} />
      ...
      case FIELD_TYPES.NUMBER:
        return <input 
          ...
          type="number"
          onChange={(e) => debouncedHandleChange(Number(e.target.value), field.slug)}
        />
      default:
        return <input defaultValue={defaultFiltersMap[field.slug]} placeholder="Filter by" type="text" onChange={(e) => debouncedHandleChange(e.target.value, field.slug)} />
    }
  }
